  ...
  namespace eval EXT {
      ...
      if {[file exists  ${TE::PREBUILT_HW_PATH}/${int_shortdir}/${TE::VPROJ_NAME}.mmi]} { 
        ...
        foreach t $tmp {
          if {[string match *InstPath=* $t] } {
            ...
            set hitval [string map {"\"" ""} $hitval]
            if { $hitval eq "NA"} {
              TE::UTILS::te_msg TE_SW-6 ERROR "Processor doesn't exist in ${TE::PREBUILT_HW_PATH}/${int_shortdir}/${TE::VPROJ_NAME}.mmi."
              return -code error "Processor doesn't exist in ${TE::PREBUILT_HW_PATH}/${int_shortdir}/${TE::VPROJ_NAME}.mmi.";
            }
          }
        }
        #---------------
              ...
              while {$data_index < [llength $sw_applist_line] } {
                ...
                if {[lindex $sw_applist_line $data_index] ne "NA"} {
                  ...
                  if {![file exists ${TE::BASEFOLDER}/${test_path}]} { 
                    set temp_path "[lindex $sw_applist_line $data_index]"
                    set temp_path [TE::UTILS::regex_map $temp_path "<short_dir>" "default" ]
                  }
                  #check if srec is required and generate srec from elf
                  if {[string match *.srec* $temp_path]} {
                    set elf_path [TE::UTILS::regex_map $temp_path ".srec" ".elf" ]
                    generate_srec ${rel_data_file}/$elf_path ${rel_data_file}/$temp_path
                  }
                  # set load_data "$load_data up [lindex $sw_applist_line [expr $data_index+1]] ${rel_data_file}/[lindex $sw_applist_line $data_index] "
                  set load_data "$load_data up [lindex $sw_applist_line [expr $data_index+1]] ${rel_data_file}/${temp_path} "
                }
    ...
    proc generate_bif_files {{fname ""}} {
      ...
      if {![file exists ${TE::PREBUILT_HW_PATH}/${int_shortdir}/${TE::VPROJ_NAME}.bit]} { 
        ...
        # default  not found
          TE::UTILS::te_msg TE_SW-11 ERROR "Bit-file (${TE::PREBUILT_HW_PATH}/${int_shortdir}/${TE::VPROJ_NAME}.bit or ${TE::PREBUILT_HW_PATH}/default/${TE::VPROJ_NAME}.bit) doesn't exist."
          return -code error "Project bit-file (${TE::PREBUILT_HW_PATH}/${int_shortdir}/${TE::VPROJ_NAME}.bit or ${TE::PREBUILT_HW_PATH}/default/${TE::VPROJ_NAME}.bit) doesn't exist.";
        } else {
        ...
        if {[lindex $sw_applist_line 2] eq "FSBL" || [lindex $sw_applist_line 2] eq "FSBL_EXT"} {
          ...
          if {![file exists ${TE::PREBUILT_SW_PATH}/${int_shortdir}/${fsbl_name}.elf]} { 
            ...
            # default fsbl not found
              TE::UTILS::te_msg TE_SW-12 ERROR "FSBL ELF-file (${TE::PREBUILT_SW_PATH}/${int_shortdir}/${fsbl_name}.elf or ${TE::PREBUILT_SW_PATH}/default/${fsbl_name}.elf) doesn't exist."
              return -code error "FSBL ELF-file (${TE::PREBUILT_SW_PATH}/${int_shortdir}/${fsbl_name}.elf or ${TE::PREBUILT_SW_PATH}/default/${fsbl_name}.elf) doesn't exist.";
            } else {
      ...
      foreach sw_applist_line ${TE::SW_APPLIST} {
        ...
        if {[lindex $sw_applist_line 2] eq "0" || [lindex $sw_applist_line 2] eq "1" || [lindex $sw_applist_line 2] eq "FSBL_APP"} {
          ...
          if {![file exists ${checkfile}/${int_shortdir}/${app_name}.elf]} { 
            ...
            # search default
            # default  not found
              TE::UTILS::te_msg TE_SW-16 ERROR "Application ELF-file (${checkfile}/${int_shortdir}/${app_name}.elf or ${checkfile}/default/${app_name}.elf) doesn't exist."
              return -code error "Application ELF-file (${checkfile}/${int_shortdir}/${app_name}.elf or ${checkfile}/default/${app_name}.elf) doesn't exist.";
            } else {
          ...
          if {$TE::IS_ZSYS} {
            #Zynq
            set data01_file [lindex $sw_applist_line 6]
            set data01_load [lindex $sw_applist_line 7]
            set data01_offset [lindex $sw_applist_line 8]
            #replace na with ""
            ...
            #uzynq
            set fsbl_config [lindex $sw_applist_line 6]
            if {[string match $fsbl_config "NA"]} { set fsbl_config ""}
            set destination_cpu [lindex $sw_applist_line 7]
            if {[string match $destination_cpu "NA"]} { set destination_cpu ""}
            set exception_level [lindex $sw_applist_line 8]
            if {[string match $exception_level "NA"]} { set exception_level ""}
            set atf  [lindex $sw_applist_line 9]
            if {[string match $atf "NA"]} { set atf ""} elseif {[file exists ${checkfile}/${int_shortdir}/${atf}]} { 
              ...
              TE::UTILS::te_msg TE_SW-17 WARNING "ATF File doesn't exist in ${checkfile}/${int_shortdir}/${atf} or ${checkfile}/default/${atf}"
             set atf ""
            }
            set pmu  [lindex $sw_applist_line 10]
            if {[string match $pmu "NA"]} { 
              ...
              TE::UTILS::te_msg TE_SW-64 WARNING "PMU File ($pmu) doesn't exist in all possible prebuilt folders"
             set pmu ""
            }
      ...
      if {${TE::ZIP_PATH} ne ""} {
        ...
        switch [TE::UTILS::get_host_os] {
          ...
          "unix" {
            ...
            - to instal zip : sudo apt install zip \n \
