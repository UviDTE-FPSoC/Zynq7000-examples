...
//					if ((l>=2) && (j==(operation_loops-1)))  update_cumulative2(&total_dma_wr, &min_dma_wr, &max_dma_wr, &variance_dma_wr);
				}
				else {
...
//			printf("Check with data size %dB was OK RD\n\r", data_size[i]);
//		} //REP_TEST
...
//			if ((l>=2) && (j==(operation_loops-1)))  update_cumulative2(&total_dma_rd, &min_dma_rd, &max_dma_rd, &variance_dma_rd);
		}
		else {
			DmaInstance.Chans[Channel].DmaCmdToHw = NULL;
			Status = XST_FAILURE;
