...
//microcode program is prepared when opening char driver if
//prepare_microcode_in_open = 1
static int prepare_microcode_in_open = 0;
//when prepare_microcode_in_open=1 the following vars are used to prepare DMA microcodes in open() func
static void* dma_buff_padd = (void*) 0x00000000;//physical address of buff to use in write and read from application
 ...
 */
static struct kobj_attribute dma_buff_padd_attr =
  __ATTR(dma_buff_padd, 0666, dma_buff_padd_show, dma_buff_padd_store);
static struct kobj_attribute prepare_microcode_in_open_attr =
  __ATTR(prepare_microcode_in_open, 0666,
  prepare_microcode_in_open_show, prepare_microcode_in_open_store);
static struct kobj_attribute dma_transfer_size_attr =
  __ATTR(dma_transfer_size, 0666,
    dma_transfer_size_show, dma_transfer_size_store);

/**  The pl330_lkm_attrs[] is an array of attributes that is used to create
 *  the attribute group below. The attr property of the kobj_attribute is used
...
/**  The attribute group uses the attribute array and a name, which is exposed
 *  on sysfs -- in this case it is gpio115, which is automatically defined in
 *  the ebbButton_init() function below using the custom kernel parameter that
 *  can be passed when the module is loaded.
 */
...
/** @brief The device open function, called each time the device is opened.
 *  This will only increment the numberOpens counter in this case and
 *  prepare DMAC microcode if prepare_microcode_in_open = 1.
...
static int dev_open(struct inode *inodep, struct file *filep){
   ...
   // A good module would get a resource de device cannot be open a second
   // time before closing it

   if (prepare_microcode_in_open == 1)
   {
      //Prepare program for writing the FPGA (WR)
      ...
      //Prepare program for reading the FPGA (RD)
  ...
  loff_t *offset)
{
  ...
  if (prepare_microcode_in_open == 1)
  {
    ...
    status = alt_dma_channel_exec(Dma_Channel,
      (ALT_DMA_PROGRAM_t*) DMA_PROG_RD_H);
  }
  else
  {
  ...
  if (status == ALT_E_SUCCESS)
  {
    while((status == ALT_E_SUCCESS) && (channel_state !=
   ...
   if (error_count!=0){ // if true then have success
      printk(KERN_INFO
        "DMA LKM: Failed to send %d characters to the user in read function\n",
        error_count);
  ...
  loff_t *offset)
{
  ...
  if (error_count!=0){ // if true then have success
    printk(KERN_INFO
      "DMA LKM: Failed to copy %d characters from the user in write function\n",
      error_count);
    return -EFAULT;  // Failed -- return a bad address message (i.e. -14)
  }

  //Copy data from the application memory to the FPGA
  if (prepare_microcode_in_open == 1)
  {
    //execute the program prepared in the open
    status = alt_dma_channel_exec(Dma_Channel,
